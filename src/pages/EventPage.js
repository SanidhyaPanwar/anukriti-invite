  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        ...
        const foundEvent = eventsRes.data.find(e => 
          e._id === eventId || 
          e.title.toLowerCase().includes(eventId)
        );
        setEvent(foundEvent);
      } catch (err) {
        console.error(err);
        ...
        <Fade in timeout={1000}>
          ...
          >
            ...
            <Box sx={{ textAlign: 'center', mb: 4 }}>
              <Typography variant="h4" gutterBottom>{event.date}</Typography>
              <Typography variant="h5" color="text.secondary">{event.time}</Typography>
            </Box>

            {/* REMOVED CardMedia - NO MORE BROKEN IMAGES */}
            ...
            <Box sx={{ 
              ...
              <Typography variant="h6" sx={{ mb: 1, fontWeight: 'bold', color: theme.color }}>
                📍 {event.venue}
              </Typography>
            </Box>
