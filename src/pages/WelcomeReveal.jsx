...
export default function WelcomeReveal() {
  ...
  const firstBlockParents = isBrideSide 
    ? 'Daughter of Mrs. Sanju & Dr. Indra Pal Singh' 
  ...
  // Helper for Cover & Card 1: Uses "& Family" instead of individual names
  const getCoverGuestName = () => {
    if (!guest) return '';
    if (guest.withFamily) {
      return `${guest.fullName} & Family`;
    }
  ...
  const getFamilyMentionString = () => {
    ...
    if (guest.familyMembers && guest.familyMembers.length > 0) {
      ...
      return ` along with ${names.slice(0, -1).join(', ')}, and ${names[names.length - 1]}`;
    }
    if (guest.withFamily) {
  ...
  const smoothScrollTo = (endY, duration) => {
    ...
    const animation = (currentTime) => {
      if (userInteractedRef.current) { cancelAnimationFrame(animationFrameId); return; }
  ...
  const setupScratchCanvas = (node, heartIndex) => {
    ...
    node.addEventListener('touchstart', () => { isDrawing = true; }, { passive: true });
  ...
  const triggerRefinedConfetti = () => {
    ...
    (function frame() {
      confetti({ particleCount: 3, angle: 60, spread: 45, origin: { x: 0, y: 0.6 }, colors, zIndex: 9999 });
      confetti({ particleCount: 3, angle: 120, spread: 45, origin: { x: 1, y: 0.6 }, colors, zIndex: 9999 });
  ...
  const heartMaskStyle = {
    maskImage: "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 200 185' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M100 185 C100 185, 200 125, 200 70 C200 23, 157 -10, 121.5 15 C100 31, 100 31, 100 31 C100 31, 100 31, 78.5 15 C43 -10, 0 23, 0 70 C0 125, 100 185, 100 185 Z' fill='black'/%3E%3C/svg%3E\")",
    WebkitMaskImage: "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 200 185' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M100 185 C100 185, 200 125, 200 70 C200 23, 157 -10, 121.5 15 C100 31, 100 31, 100 31 C100 31, 100 31, 78.5 15 C43 -10, 0 23, 0 70 C0 125, 100 185, 100 185 Z' fill='black'/%3E%3C/svg%3E\")",
    maskSize: 'contain',
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#130915] text-[#D4AF37]">
        <p className="font-script text-5xl animate-pulse tracking-wider">Loading  Festivities...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#130915] p-4">
        <div className="bg-[#FAF8F5] p-8 rounded-3xl shadow-2xl text-center max-w-md border border-gold/40">
          <h2 className="text-xl font-serif font-semibold text-gray-900 mb-2">Invitation Not Found</h2>
          <p className="text-gray-600 text-sm">Please verify your unique access link from the host.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen bg-[#130915] flex flex-col items-center justify-center p-4 relative select-none ${!curtainsParted ? 'overflow-hidden' : 'overflow-y-auto pb-24'}`}>
      
      {/* FALLING PETALS CANVAS (Strictly Background z-0) */}
      <canvas id="petals-canvas" className="fixed inset-0 pointer-events-none z-0" />
      ...
      <AnimatePresence>
        {!hasEntered && (
          ...
          >
            ...
            <img src={ganeshImg} alt="Shri Ganesh" className="h-14 w-auto object-contain mx-auto mb-3" />
            </div>
            <p className="text-lavender-200 uppercase tracking-[0.3em] text-xs font-semibold mb-8">वक्रतुण्ड महाकाय सूर्यकोटि समप्रभ</p>
      ...
      <AnimatePresence>
        {hasEntered && !curtainsParted && (
          <motion.div className="fixed inset-0 z-50 flex overflow-hidden pointer-events-none" exit={{ opacity: 0 }} transition={{ duration: 0.8, delay: 1.0 }}>
            <motion.div initial={{ x: 0 }} animate={{ x: '-100%' }} transition={{ duration: 1.8, ease: [0.77, 0, 0.175, 1] }} className="w-1/2 h-full bg-[#2c1238] border-r-4 border-gold shadow-[15px_0_30px_rgba(0,0,0,0.6)] relative bg-[radial-gradient(ellipse_at_right,_var(--tw-gradient-stops))] from-[#4b1d5e] via-[#2c1238] to-[#1a0922]" />
            <motion.div initial={{ x: 0 }} animate={{ x: '100%' }} transition={{ duration: 1.8, ease: [0.77, 0, 0.175, 1] }} className="w-1/2 h-full bg-[#2c1238] border-l-4 border-gold shadow-[-15px_0_30px_rgba(0,0,0,0.6)] relative bg-[radial-gradient(ellipse_at_left,_var(--tw-gradient-stops))] from-[#4b1d5e] via-[#2c1238] to-[#1a0922]" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="relative z-10 w-full max-w-xl my-10 space-y-12">
        <AnimatePresence mode="wait">
          ...
          {!invitationOpened ? (
            ...
            <motion.div 
              key="cover"
              initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 1.05, y: -20 }} transition={{ duration: 0.8, delay: 0.6 }} 
            ...
            >
              <img src={ganeshImg} alt="Shri Ganesh" className="h-14 w-auto object-contain mx-auto mb-3" />
              <span className="text-[10px] uppercase tracking-[0.4em] text-amber-800 font-bold mb-2 block">Wedding Invitation</span>
              
              <h1 className="font-script text-5xl md:text-6xl text-lavender-950 mb-4 font-bold">
                {firstCoupleName} <span className="text-amber-600 font-serif text-3xl">&</span> {secondCoupleName}
              </h1>

              <div className="py-3 px-6 bg-lavender-50 border border-gold/30 rounded-2xl my-3 shadow-inner w-full max-w-sm">
                <span className="text-[10px] uppercase tracking-widest text-amber-800 font-bold block mb-0.5">With warmth and joy, cordially invited</span>
                <p className="text-xl font-bold text-gray-900 capitalize font-serif">{getCoverGuestName()}</p>
              </div>

              <button onClick={() => setInvitationOpened(true)} className="w-full bg-gradient-to-r from-lavender-900 via-lavender-950 to-lavender-900 text-gold-light border border-gold/40 py-4 px-6 rounded-2xl font-semibold flex items-center justify-center gap-3 shadow-xl transition-all text-sm tracking-widest uppercase cursor-pointer hover:scale-[1.02] mt-4">
            ...
            <div className="space-y-8 w-full">
              ...
              <motion.div 
                initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8 }}
                className="w-full bg-[#FAF8F5] rounded-3xl shadow-2xl border-4 border-gold/50 p-6 md:p-10 text-center relative"
              >
                <img src={ganeshImg} alt="Shri Ganesh" className="h-14 w-auto object-contain mx-auto mb-2" />
                ...
                {/* First Block */}
                <div className="my-2">
                  <span className="font-script text-5xl md:text-6xl text-lavender-950 block">{firstBlockName}</span>
                  <p className="font-serif text-sm font-semibold text-gray-900 mt-1">{firstBlockParents}</p>
                </div>
                ...
                {/* Second Block */}
                <div className="my-2">
                  <span className="font-script text-5xl md:text-6xl text-lavender-950 block">{secondBlockName}</span>
                  <p className="font-serif text-sm font-semibold text-gray-900 mt-1">{secondBlockParents}</p>
                </div>
              </motion.div>
              ...
              <motion.div 
                ...
                initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
                className="w-full bg-[#FAF8F5] rounded-3xl shadow-2xl border-4 border-gold/50 p-6 md:p-10 text-center relative"
              >
                <span className="text-xs uppercase tracking-[0.3em] text-amber-800 font-bold block mb-1">Save The Date</span>
                <h2 className="font-script text-5xl text-lavender-950 mb-2">Reveal Our Big Day</h2>
                <p className="text-xs text-gray-500 italic mb-6">Scratch the heart cards below to reveal date details</p>

                <div className="flex justify-center items-center gap-4 my-6">
                  
                  {/* Heart 1: DAY */}
                  <div style={heartMaskStyle} className="relative w-[28vw] max-w-[120px] aspect-[1.1/1] bg-lavender-50 flex flex-col items-center justify-center shadow-lg border border-gold/30">
                    <span className="text-[9px] uppercase text-amber-800 font-bold tracking-widest">DAY</span>
                    <span className="text-2xl font-serif font-bold text-lavender-950">10</span>
                    <canvas ref={(node) => setupScratchCanvas(node, 1)} style={heartMaskStyle} className="absolute inset-0 w-full h-full cursor-crosshair z-20 touch-none" />
                  </div>

                  {/* Heart 2: MONTH */}
                  <div style={heartMaskStyle} className="relative w-[28vw] max-w-[120px] aspect-[1.1/1] bg-lavender-50 flex flex-col items-center justify-center shadow-lg border border-gold/30">
                    <span className="text-[9px] uppercase text-amber-800 font-bold tracking-widest">MONTH</span>
                    <span className="text-xl font-serif font-bold text-lavender-950">Dec</span>
                    <canvas ref={(node) => setupScratchCanvas(node, 2)} style={heartMaskStyle} className="absolute inset-0 w-full h-full cursor-crosshair z-20 touch-none" />
                  </div>

                  {/* Heart 3: YEAR */}
                  <div style={heartMaskStyle} className="relative w-[28vw] max-w-[120px] aspect-[1.1/1] bg-lavender-50 flex flex-col items-center justify-center shadow-lg border border-gold/30">
                    <span className="text-[9px] uppercase text-amber-800 font-bold tracking-widest">YEAR</span>
                    <span className="text-xl font-serif font-bold text-lavender-950">2026</span>
                    <canvas ref={(node) => setupScratchCanvas(node, 3)} style={heartMaskStyle} className="absolute inset-0 w-full h-full cursor-crosshair z-20 touch-none" />
                  </div>
                ...
                {allHeartsRevealed && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 space-y-4">
                    <p className="font-script text-3xl text-amber-800">The start of a beautiful journey...</p>
                    <div className="grid grid-cols-4 gap-2 w-full max-w-sm mx-auto">
                      {[
                        ...
                        <div key={idx} className="bg-white border border-gold/30 rounded-xl py-2.5 flex flex-col items-center shadow-inner">
                          <span className="text-xl font-serif font-bold text-lavender-950">{item.value}</span>
                          <span className="text-[9px] font-semibold text-gray-500 tracking-widest">{item.label}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
              ...
              >
                ...
                <div className="space-y-2">
                  <span className="text-[10px] uppercase tracking-[0.3em] text-amber-800 font-bold block">Special Invitation For</span>
                  <h3 className="text-2xl md:text-3xl font-serif font-bold text-lavender-950">
                    Dearest {guest.fullName},
                  </h3>
                </div>
                ...
                <div className="font-serif italic text-sm md:text-base text-gray-700 leading-relaxed px-4 py-4 bg-lavender-50/60 rounded-2xl border border-gold/20 shadow-inner">
                  <p>
                    When we think about our wedding day, we don't just think of the celebrations; we think of the people who mean the absolute world to us. Having you{getFamilyMentionString()} with us to share the joy, the rituals, and the warmth of this new beginning would make our happiness complete.
                  </p>
                  <p className="mt-3">
                    We can't wait to create cherished memories and celebrate together!
                  </p>
                  <p className="mt-4 font-script text-2xl text-amber-800 not-italic">
                    With love, Dr. Indra Pal Singh and Family
                  </p>
                </div>
                ...
                {guest.photos && guest.photos.length > 0 && (
                  <div className="p-4 bg-white/70 backdrop-blur-sm rounded-2xl border border-gold/30 mt-4">
                    <h4 className="text-xs uppercase tracking-widest text-lavender-950 font-bold mb-3 flex items-center justify-center gap-1.5">
                      <ImageIcon className="text-amber-600" size={14} /> Cherished Memories Together
                    </h4>
                    <div className="grid grid-cols-2 gap-3">
                      {guest.photos.map((photoUrl, idx) => (
                        <div key={idx} className="relative rounded-2xl overflow-hidden shadow-sm border border-gold/30 aspect-square">
                          <img src={photoUrl} alt="Memory" className="w-full h-full object-cover transform hover:scale-110 transition duration-500" />
                        </div>
                      ))}
                    </div>
                  </div>
