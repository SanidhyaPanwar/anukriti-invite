...
import { 
  ...
  Tabs, Tab, AppBar, Fade, CircularProgress 
} from '@mui/material';
...
function InvitePage() {
  ...
  if (error || !guest) {
    return (
      <Container maxWidth="sm" sx={{ py: 8, textAlign: 'center' }}>
        <Typography variant="h5" color="error">{error || "Guest not found"}</Typography>
      </Container>
    );
  }

  const honorific = getHonorific(guest.gender);
  const guestName = `${honorific} ${guest.fullName}${guest.withFamily ? ' and Family' : ''}`;
  ...
  return (
    <Box sx={{ 
      ...
      >
        ...
        <Box sx={{ 
          bgcolor: 'linear-gradient(135deg, #E1BEE7 0%, #CE93D8 50%, #BA68C8 100%)',  // ← Lavender gradient
          color: '#2c1810',
          p: 6,
          textAlign: 'center',
          position: 'relative'
        ...
        <Box sx={{ p: 6, textAlign: 'center', bgcolor: '#F3E5F5' }}>  {/* ← Light lavender */}
          <Typography 
            variant="h4" 
            sx={{ 
              fontFamily: 'Playfair Display, serif',
        ...
        <Box sx={{ p: 6, textAlign: 'center' }}>
          ...
          >
            It gives us <strong>immense pleasure</strong> to invite you<br/>
            to grace the wedding ceremony of
          </Typography>
          
          <Divider sx={{ my: 3, mx: 'auto', width: '80%', bgcolor: '#AB47BC' }} />  {/* ← Lavender pink */}
          ...
          >
            and bless the couple on this <strong>auspicious occasion</strong>.<br/>
            We seek your <strong>presence and blessings</strong>.
          </Typography>
          
          <Divider sx={{ my: 3, mx: 'auto', width: '60%', bgcolor: '#CE93D8' }} />  {/* ← Medium lavender */}
      ...
      <Container maxWidth="md" sx={{ mt: 8, pb: 8 }}>
        ...
        {activeTab === 0 && <ProgrammesTab events={events} inviteCode={inviteCode} />}
